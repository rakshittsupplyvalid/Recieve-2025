...
const OTPLoginScreen = () => {
    ...
    try {
      ...
      if (registered) {
        SmsRetriever.addSmsListener(event => {
          ...
          const otpMatch = event.message.match(/(\d{4,6})/); // Extract OTP
          if (otpMatch) {
            setOtp(otpMatch[0]);
            validateOTP(otpMatch[0]);
          }
          SmsRetriever.removeSmsListener();
        });
      }
    } catch (error) {
      console.warn('Error starting SMS listener:', error);
    }
  };

  const validateOTP = (receivedOtp) => {
    // LOCAL VALIDATION
    const originalOtp = '123456'; // You can store this locally if generated on device
    if (receivedOtp === originalOtp) {
  ...
  return (
    <View style={{ padding: 20 }}>
      ...
      <Text style={{ marginTop: 20 }}>Enter OTP (optional):</Text>
      <TextInput
        value={otp}
        onChangeText={setOtp}
